(function (angular, _) {
     ...
     * @ngdoc module
     *
     * @name huqas.auth.services
     *
     * @description
     * Module containing common auth services
     */

    angular.module("huqas.auth.services", [
        "api.wrapper",
        "huqas.auth.oauth2",
        "huqas.auth.permissions",
        "ui.router",
        "ngIdle"
    ])
     ...
     * @ngdoc service
     *
     * @name huqas.auth.services.login
     *
     * @description
     * Auth login service
     * Handles logins, logouts, fetching current user
     */
    .service("huqas.auth.services.login", ["api", "$window", "$q", "api.oauth2", "Idle", "Title",
        function (API, $window, $q, oauth2, Idle, Title) {
            var url = {
                curr_user : "api/v1/users/me/"
            };
            var store_keys = {
                "user": "auth.user",
                "state": "state.dump"
     ...
     * @ngdoc service
     *
     * @name huqas.auth.services.statecheck
     *
     * @description
     * Handles various state management issues e.g. authorization, redirection
     */
    .service("huqas.auth.services.statecheck",
        ["$rootScope", "$injector", "huqas.auth.services.login",
        "huqas.auth.permissions.checker", "HOME_PAGE_NAME",
     ...
     * @ngdoc service
     *
     * @name huqas.auth.services.profile
     *
     * @description
     * Handles current user's profile
     */
    .service("huqas.auth.services.profile", ["$q", "api", function ($q, API) {
        var urls = {
